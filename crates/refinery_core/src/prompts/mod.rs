...
pub fn wrap_answer(answer: &str, model_label: &str, nonce: &str) -> String {
    ...
    let sanitized_label = escape_xml_attr(model_label);
    format!("<answer-{nonce} model=\"{sanitized_label}\">\n{sanitized}\n</answer-{nonce}>")
}
...
pub fn shuffled_labels(count: usize) -> Vec<String> {
    let mut labels: Vec<String> = (0..count)
        .map(|i| {
            let c =
                char::from(b'A' + u8::try_from(i % 26).expect("label index modulo 26 fits in u8"));
            format!("Answer {c}")
        })
        .collect();
...
// Re-export JSON utilities from tundish_core
pub use tundish_core::{check_json_depth, extract_json};
