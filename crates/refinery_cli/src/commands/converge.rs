...
pub async fn run(args: ConvergeArgs) -> ExitCode {
    ...
    if args.stability_rounds > args.max_rounds {
        emit_validation_error(
            shared,
            &format!(
                "--stability-rounds ({}) cannot exceed --max-rounds ({})",
    ...
    if shared.dry_run {
            ...
            .collect::<Result<Vec<_>, _>>()
        {
            Ok(ids) => ids,
            Err(e) => {
                emit_validation_error(shared, &e);
                return ExitCode::from(4);
            }
        };
        let config = match EngineConfig::new(
            ...
            Ok(c) => c,
            Err(e) => {
                emit_validation_error(shared, &format!("Config error: {e}"));
                return ExitCode::from(4);
            }
        };
...
fn emit_validation_error(shared: &SharedArgs, message: &str) {
    match shared.output_format {
        OutputFormat::Json => {
            let err_response = ErrorResponse {
                status: "error".to_string(),
