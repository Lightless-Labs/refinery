...
/// - Strips empty and relative segments to reduce PATH-hijack risk.
...
pub(crate) fn unique_temp_path(prefix: &str, extension: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "{prefix}-{}-{count}.{extension}",
        std::process::id()
    ))
}

/// Resolve a CLI binary to its absolute path using the same sanitized PATH
/// passed to child processes.
pub fn resolve_binary(name: &str) -> Result<PathBuf, ProviderError> {
    for dir in std::env::split_paths(&sanitized_path()) {
...
pub fn extract_gemini_response(json_text: &str) -> Result<String, ProviderError> {
    ...
    if let Some(inner) = tundish_core::extract_json(response) {
        Ok(inner.to_string())
    } else {
...
{"type":"turn.started","turn_id":"u1"}"#;
    ...
    async fn resolve_binary_not_found() {
        let result = resolve_binary("definitely_not_a_real_binary_xyz123");
