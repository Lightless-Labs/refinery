...
- **013** — brainstorm strategy benchmarks (in progress): phase-1 benchmark design and `refinery benchmark-brainstorm` artifact analyzer completed; next step is a 6-prompt v0 baseline suite, then benchmark-only iteration variants
...
2. If continuing `todos/013`, run the 6-prompt v0 baseline suite and analyze it with `refinery benchmark-brainstorm` before adding new strategy variants.
