...
status: completed
date: 2026-03-31
enhanced: 2026-04-24 — divergence expansion notes from Open Collider + prompt-reframing discussion
origin: docs/brainstorms/2026-03-30-brainstorm-verb-requirements.md
---

# Brainstorm Verb

**Enhanced:** 2026-04-24 — added post-v0 divergence expansion notes for prompt reframing and Open Collider-style domain collisions.
...
## Addendum: Divergence Expansion Strategies (2026-04-24)

The v0 brainstorm design already optimizes for divergence by avoiding consensus pressure: models iterate independently, receive only their own prior answers plus aggregate scores, and final selection favors quality plus evaluator disagreement. The Open Collider discussion adds a separate insight: v0 **preserves** divergence, but future strategies can also **inject** divergence upstream.

### Prompt Reframing Expansion

Before the first brainstorm round, ask each model to produce one strategically different variation of the original brief. These should not be paraphrases; they should alter the problem frame by changing assumptions, success criteria, time horizon, neglected stakeholders, causal model, constraint set, risk appetite, or abstraction level.

Then every model works independently on both the original prompt and every model-generated variation:

```text
lineages = n models × (1 original prompt + n prompt variations)
         = n(n + 1) lineages
```

For example, 3 models yield 12 lineages before any optional domain-collision expansion. The original prompt remains as the anchor so the search does not drift entirely away from the user's intent.

### Domain Collision Expansion

Open Collider-style bisociation suggests a complementary divergence primitive: generate structurally distant domains and force isolated brief × domain collisions. Each domain should carry a specialist persona, a counterintuitive mechanism, and a bridge question into the user's problem.

With prompt reframing and domain collisions combined:

```text
lineages = n models × (1 original + p prompt variations) × d domains
```

If `p = n`, this becomes `n(n + 1)d`, which is powerful but expensive.

### Budget and Evaluation Implications

Full expansion explodes proposal and evaluation calls. Future implementations need explicit controls before enabling this broadly:
...
Selection may also need to evolve beyond `mean * stddev` once many lineages exist. Candidate future composite scoring:
...
Do not replace v0's score-only controversial brainstorm with these mechanisms. Treat them as post-v0 strategy variants to benchmark against the v0 baseline. See `todos/018-brainstorm-divergence-expansion.md` and `todos/013-brainstorm-strategy-benchmarks.md`.
...
- **Divergence expansion TODO:** `todos/018-brainstorm-divergence-expansion.md`
- **Synthesize verb (template):** `crates/refinery_cli/src/commands/synthesize.rs`
- **Open Collider:** `https://github.com/CL-ML/open-collider` — referenced for bisociation/domain-collision strategy inspiration
